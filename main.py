...
def do_sign():
    ...
    print(f'[{get_current_time()}]' + f'开始执行签到操作...')
    ...
    dosign_response = json_handel(res_dosign.text)
    ...
    balance_response = json_handel(res_balance.text)
    ...
    if resultmsg == 'SUCCESS':
        ...
        print(
            f'[{get_current_time()}]' + f'签到成功力,本次签到获得的积分为{acquireintegration}' + f'总积分为{balance}.')
    elif resultmsg == '今日已签到，请勿重复签到':
        print(f'[{get_current_time()}]' + f'你已经签到过了,你是小猪吗?, 总积分为{balance}.')
    else:
        print(f'[{get_current_time()}]' + f'发生了终极大错误,重新执行一下,不行反馈给开发者.')
